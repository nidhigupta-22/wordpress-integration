	...
	class KayakoApi
	{
		...
		public function CreateTicketRESTAPI($_fullName, $_email, $_contents, $_subject, $_departmentID, $_ticketStatusID, $_ticketPriority, $_ticketTypeID)
		{
			if (!empty($_fullName) && !empty($_email) && !empty($_contents) && !empty($_subject) && !empty($_departmentID) && !empty($_ticketStatusID) && !empty($_ticketPriority) && !empty($_ticketTypeID)) {
				...
				$_data = array(
					"subject"        => $_subject, "fullname" => $_fullName, "email" => $_email, "contents" => $_contents, "departmentid" => $_departmentID,
					"ticketstatusid" => $_ticketStatusID, "ticketpriorityid" => $_ticketPriority, "tickettypeid" => $_ticketTypeID, "autouserid" => '1', "salt" => $this->salt,
					"signature"      => $this->encodedSignature, "apikey" => $this->apiKey
				);
		...
		public function UpdateTicket($_getValues)
		{
			if (is_array($_getValues)) {
				...
				$updateTicketdetails = array(
					"ticketstatusid" => $_getValues['statusid'], "ticketpriorityid" => $_getValues['priorityid'], "salt" => $this->salt, "signature" => $this->encodedSignature,
					"apikey"         => $this->apiKey
				);
		...
		public function CreateTicketPost($ticketid, $contents, $userid)
		{
			if (!empty($ticketid) && !empty($contents) && !empty($userid)) {
				...
				$_dataDetails = array(
					"ticketid" => $ticketid, "contents" => $contents, "userid" => $userid, "salt" => $this->salt, "signature" => $this->encodedSignature, "apikey" => $this->apiKey
				);
