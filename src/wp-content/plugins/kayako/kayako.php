...
final class Kayako
{
	...
	public function kayako_admin_init()
	{
		...
		add_settings_section('kayako_livechat_tag_form', __('Live Chat tag generator', 'kayako'), array(
			&$this, 'livechat_tag_settings_description'
		), 'kayako-livechat_tag-settings');
		add_settings_field('kayako_tag', __('Paste the tag generated by Kayako fusion here : ', 'kayako'), array(
			&$this, '_livechat_tag_textarea'
		), 'kayako-livechat_tag-settings', 'kayako_livechat_tag_form');
	...
	{
		add_menu_page('Kayako for Wordpress', 'Kayako', 'manage_options', 'kayako-support', array(
			&$this, '_admin_menu_contents'
		), plugins_url('kayako/images/small_esupport.gif'), 99);
		...
		add_submenu_page('kayako-support', 'Kayako Livechat Tag', __('LiveChat Tag', 'support'), 'manage_options', 'kayako-livechat_tag-settings', array(
						...
						<td><select name="department_type" onchange="this.form.submit()">
								...
								<?php foreach ($this->getDepartments() as $key => $val) {
									if ($val['module'] == 'tickets' && $val['type'] == 'public') { ?>
										<option value="<?php echo base64_encode($val['id']); ?>" <?php selected($val['id'] == $departmentID); ?>> - <?php echo $val['title']; ?>-
										</option>
									<?php }
								} ?>
							</select>
					...
					foreach ($result['result'] as $key => $getvalue) {
						...
						foreach ($getvalue as $key2 => $ticketvals) {
							...
							if ($ticketvals['departmentid'] == $departmentID && $ticketvals['departmentid'] != '0') {
								...
								?>
								<tr>
									<td class="white" align="center"><a class="post_reply" href="javascript:void();"
																		kayako_post_reply="<?php echo $ticketvals['displayid']; ?>"><?php echo $ticketvals['displayid']; ?></a></td>
									...
									<td style="background-color: <?php echo $_priorityProperties['bgcolorcode']; ?>" align="center"><span
											style="color:<?php echo $_priorityProperties['frcolorcode']; ?>"><?php echo $_priorityProperties['title'] ?></span></td>
									<td style="background-color: <?php echo $_statusProperties['statusbgcolor']; ?>" align="center"><span
											style="color:<?php echo $_statusProperties['statuscolor']; ?>"><?php echo $_statusProperties['title'] ?></span></td>
								</tr>
	...
	public function ticketPostDiv()
	{
		...
		if (!$_returnTicketStatuses['errorMessage']) {
			foreach ($_returnTicketStatuses as $key => $val) {
				if ($val['type'] == 'public') {
					...
					if ($result['result']['ticket']['0']['statusid'] == $val['id']) {
			...
			echo "<div class = 'ky_small_text'>No data found</div>";
		}
		$html .= '</td>
		...
		if (!$_returnTicketPriority['errorMessage']) {
			foreach ($_returnTicketPriority as $key => $val) {
				if ($val['type'] == 'public') {
					$html .= '<option value="' . $val['id'] . '" ';
					if ($result['result']['ticket']['0']['priorityid'] == $val['id']) {
						$html .= 'selected=selected';
					}
					$html .= '>' . $val['title'] . '</option>';
				}
	...
	<div class="wrap">
        ...
        <h2><?php if ($commentData) {
		_e('Convert this comment into kayako ticket', 'kayako');
	} else {
		_e('Convert Comment to kayako ticket', 'kayako');
	} ?></h2>
